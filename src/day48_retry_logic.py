"""
Day 48 -- retry logic for API failures and malformed JSON.
"""
...
def get_structured_review(client, changed_name, context, chunk_by_name, max_retries=3):
    ...
    for attempt in range(1, max_retries + 1):
        ...
        except Exception as error:  # covers rate limits, timeouts, connection errors
            print(f"  [warning] API call failed (attempt {attempt}/{max_retries}): {error}")
            time.sleep(2 * attempt)  # back off longer each retry
            continue
